...
W 186'''
    ...
    def readFromFile(self):
        f = open('SpectralAlignment.txt', 'r')
        data = []
        for line in f:
    ...
    def constructAlignment(self, peptide, sVector, k, aaDict):
        prefixMasses = [0] + [sum([aaDict[aa] for aa in peptide[:i+1]])
                              for i in range(len(peptide))]
        diff = {prefixMasses[i+1]: (prefixMasses[i+1]-prefixMasses[i])
                for i in range(len(peptide))}
        score = dict()
        score[(0, 0, 0)] = 0
        backtrack = dict()
        ...
        for t in range(1, k+1):
            for i in prefixMasses[1:]:
                for j in range(1, len(sVector)):
                    prevList = [(i-diff[i], j-diff[i], t)] + \
                        [(i-diff[i], j1, t-1) for j1 in range(j)]
                    prevIndex = np.argmax(
                        [sVector[j] + self.getScore(node, score) for node in prevList])
                    score[(i, j, t)] = sVector[j] + \
                        self.getScore(prevList[prevIndex], score)
