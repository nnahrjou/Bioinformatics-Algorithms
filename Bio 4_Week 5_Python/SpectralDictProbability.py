...
W 186'''
    ...
    def dictProb(self, sVector, threshold, maxScore, massList):
        prob = dict()
        prob[(0, 0)] = 1
        p = sum([self.getProb(len(sVector)-1, t, sVector, massList, prob)
    ...
    def getProb(self, i, t, sVector, massList, prob):
        ...
        if i < 0 or t < 0:
            prob[(i, t)] = 0
            return 0
        p = sum([self.getProb(i-m, t-sVector[i], sVector,
                massList, prob)/20 for m in massList])
        prob[(i, t)] = p
        return p
