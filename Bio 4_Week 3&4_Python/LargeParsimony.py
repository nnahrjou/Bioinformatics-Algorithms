...
class LargeParsimony:
    ...
    def runSmallParsimony(self, n, adj, nodes, lastEdge):  # for unrooted binary tree
        ...
        for i in range(len(nodes[0])):
            s += self.singleSmallParsimony(n, adjC,
                                           adjP, adj, nodes, char2ind, ind2char, i)
    ...
    def runNearestNeighborInterchange(self, n, adj, nodes, lastEdge):
        trees = []
        score = np.inf
        newScore, newAdj, newNodes = self.runSmallParsimony(
            n, adj, deepcopy(nodes), lastEdge)
            ...
            for v in range(n, len(adj)):
                for u in adj[v].keys():
                    if u >= n and not (v, u) in visited:
                        ...
                        neighborScore, neighborAdj, neighborNodes = self.runSmallParsimony(
                            n, adj1, deepcopy(nodes), [v, u])
                        if neighborScore < newScore:
                            newScore = neighborScore
                            newAdj = neighborAdj
                            newNodes = neighborNodes
                        neighborScore, neighborAdj, neighborNodes = self.runSmallParsimony(
                            n, adj2, deepcopy(nodes), [v, u])
                        if neighborScore < newScore:
                            newScore = neighborScore
                            newAdj = neighborAdj
                            newNodes = neighborNodes
