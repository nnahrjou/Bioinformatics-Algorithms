...
'''
    ...
    def LCSBackTrack(self, v, w, sigma=11, epsilon=1, inf=1000000):
        ...
        n = len(v)
        m = len(w)
        sLower = np.matrix(-inf*np.ones((n+1)*(m+1),
                           dtype=np.int).reshape((n+1, m+1)))
        sMiddle = np.matrix(-inf*np.ones((n+1)*(m+1),
                            dtype=np.int).reshape((n+1, m+1)))
        sUpper = np.matrix(-inf*np.ones((n+1)*(m+1),
                           dtype=np.int).reshape((n+1, m+1)))
        s = [sLower, sMiddle, sUpper]
        backtrackLower = np.matrix(
            np.zeros((n+1)*(m+1), dtype=np.int).reshape((n+1, m+1)))
        backtrackMiddle = np.matrix(
            np.zeros((n+1)*(m+1), dtype=np.int).reshape((n+1, m+1)))
        backtrackUpper = np.matrix(
            np.zeros((n+1)*(m+1), dtype=np.int).reshape((n+1, m+1)))
...
if __name__ == "__main__":
    ...
    # seq1 = "PRTEINS"      # Replace this with your first input sequence
    # seq2 = "PRTWPSEIN"    # Replace this with your second input sequence

    # global_lcs = GlobalLCSGapPenalties()
    # max_score, s1, s2 = global_lcs.computeCLS(seq1, seq2)
