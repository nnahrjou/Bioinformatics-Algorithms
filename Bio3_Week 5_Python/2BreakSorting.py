...
class TwoBreakSorting:
    ...
    def printChromosome(self, chromosome):
        print('('+' '.join(['+'+str(e) if e > 0 else str(e)
              for e in chromosome])+')')
    ...
    def printGenome(self, genome):
        result = ''
        for chromosome in genome:
            result += '('+' '.join(['+'+str(e) if e >
                                    0 else str(e) for e in chromosome])+')'
        print(result)
        return result
    ...
    def shortestRearrangement(self, P, Q):
        ...
        hasNontrivialCycle, removedRedEdges = self.edgeFromNontrivialCycle(
            breakpointGraph, redEdges, blueEdges, blocks)
        while hasNontrivialCycle:
            redEdges = self.twoBreakOnGraph(
                redEdges, removedRedEdges[0][0], removedRedEdges[0][1], removedRedEdges[1][0], removedRedEdges[1][1])
            breakpointGraph = redEdges.union(blueEdges)
            P = self.twoBreakOnGenome(
                P, removedRedEdges[0][0], removedRedEdges[0][1], removedRedEdges[1][0], removedRedEdges[1][1])
            hasNontrivialCycle, removedRedEdges = self.edgeFromNontrivialCycle(
                breakpointGraph, redEdges, blueEdges, blocks)
            result.append(P)
        return result
